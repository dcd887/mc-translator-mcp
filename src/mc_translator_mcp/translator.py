...
class Translator:
    ...
    SYSTEM_PROMPT = (
        "你是一位专业的 Minecraft 游戏汉化工程师。"
        "任务：将下列英文游戏文本准确翻译成简体中文。"
        "规则：\n"
        "1. 保持 Minecraft 风格术语（如 \"Block\"→\"方块\"，\"Item\"→\"物品\"，"
        "\"Inventory\"→\"背包\"，\"Health\"→\"生命\"，\"Craft\"→\"合成\"）。"
        "2. 保留所有格式占位符：{0}、%s、$variable$、§颜色码、\\n 等绝对不能修改。\n"
        "3. 只输出翻译结果，每行格式为：<原key>:<中文翻译>，无需任何解释。\n"
        "4. 如某条目为空格或纯占位符，原样返回。\n"
        "5. 如果是专有名词（人名、地名、物品名），可参考 Minecraft 社区通用译法。"
    )
    ...
    def _call_llm(self, modid: str, batch: dict[str, str]) -> dict[str, str]:
        ...
        user_msg = (
            f"模组 ID: {modid}\n"
            f"请翻译以下条目，每行返回格式：<key>:<translation>\n"
            f"```\n{lines}\n```"
        )
        try:
